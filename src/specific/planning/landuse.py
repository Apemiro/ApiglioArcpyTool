...
def summarize_area_to_excel_2(import_landuse, landuse_fields, export_xlsx, landuse_map='TSP', **excel_options):
	LanduseMap.use_switch(landuse_map)
	area_map = {}
	...
	for row in cursor:
		...
		if landuse:
			area_map[landuse] = area_map.get(landuse, 0.0) + row[0].area
	del cursor

	# 2. 参数处理
	unit_division = {"ha": 10000.0, "hm2": 10000.0, "km2": 1000000.0}.get(excel_options.get("unit"), 10000.0)
	unit_label = excel_options.get("unit", u"公顷")
	...
	# 3. 样式定义
	wb = xlwt.Workbook(encoding='utf8')
	ws = wb.add_sheet(u"用地统计", cell_overwrite_ok=True)
			...
			if subs_d2:
				...
				for d2 in subs_d2:
					...
					if len(subs_d3) == 1 and compact:
						...
						d3_refs_for_d2 = []
						sub_start_d2 = curr_row
						d3_sum = 0.0
						for d3 in subs_d3:
							val = area_map.get(d3, 0.0)
							ws.write(curr_row, 2, get_title(d3), style_normal)
							ws.write(curr_row, 3, val / unit_division, style_num)
							d3_refs_for_d2.append(curr_row + 1)
							d3_sum += val
							curr_row += 1
